...
\newcommand{\SK}[3]{\mathcal{B}_{#1}({#2}, {#3})}			% Sum Krylov subspace
\newcommand{\mblock}[0]{{\scriptscriptstyle \square}}		% 小さな正方形
\newcommand{\BK}[3]{\mathcal{B}_{#1}^{\block}({#2}, {#3})}	% Block Krylov subspace
...
\begin{frame}[t]{}
	...
	\begin{column}{.49\linewidth}
		...
		\begin{block}{シフト線形方程式}
			\vspace{0.2\baselineskip}
			\input{./tex/02-shiftedlinear.tex}
		\end{block}
		...
		\begin{block}{一般化Lanczos過程}
			\vspace{0.2\baselineskip}
			\input{./tex/04-glanczos.tex}
		\end{block}
	\end{column}
	\hspace{0.0\columnwidth}
	\begin{column}{.49\linewidth}
		\begin{block}{Generalized shifted MINRES法}
			\input{./tex/05-gsminres.tex}
		\end{block}
		\begin{block}{数値実験}
			\input{./tex/06-experiment.tex}
		\end{block}
		\begin{block}{実験結果}
			\input{./tex/07-results.tex}
		\end{block}
