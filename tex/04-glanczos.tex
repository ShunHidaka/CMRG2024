...
\vspace{-\baselineskip}
\begin{algorithm}[H]
   \caption{\ Implementation of extend Lanczos process}
   \label{alg-imp-ex-lanczos}
   \begin{algorithmic}[1]
	\State $\beta_{0} = 0$
	\State $\vb{w}_{1} = B^{-1}\vb{b} / \norm{\vb{b}}_{B^{-1}},\ \vb{u}_{1} = \vb{b} / \norm{\vb{b}}_{B^{-1}}$
	\For {$i=1 \text{ to } j$}
		\State $\vb{u}_{i+1} = A\vb{w}_{i}$
		\State $\alpha_{i} = \inpro{\vb{w}_{i}}{\vb{u}_{i+1}}$
		\State $\vb{u}_{i+1} = \vb{u}_{i+1} - \alpha_{i}\vb{u}_{i} - \beta_{i-1}\vb{u}_{i-1}$
		\State Solve $\vb{t}$ s.t. $B\vb{w}_{i+1} = \vb{u}_{i+1}$
		\State $\beta_{i} = \norm{\vb{u}_{i+1}}_{B^{-1}} = \sqrt{ \vb{u}_{i+1}^{\htop} \vb{w}_{i+1} }$
		\State $\vb{u}_{i+1} = \vb{u}_{i+1} / \beta_{i}$
		\State $\vb{w}_{i+1} = \vb{w}_{i+1} / \beta_{i}$
	\EndFor
   \end{algorithmic}
\end{algorithm}
